...
export default function AdminDashboardClient({ userId, profile }: { userId: string, profile: any }) {
  ...
  const markAsSent = (id: string) => {
    setSentMessages(prev => new Set(prev).add(id));
  };
                              ...
                              {(() => {
                                ...
                                return (
                                  <>
                                    ...
                                    <button onClick={() => sendWhatsAppTrialWelcome(u, isUSD)} className="w-7 h-7 rounded-lg bg-yellow-500/10 flex items-center justify-center hover:bg-yellow-500 transition-all group border border-yellow-500/10" title="Mensaje de Bienvenida">
                                    ...
                                    <button onClick={() => sendWhatsAppTrialInvitation(u, isUSD)} className="w-7 h-7 rounded-lg bg-[#25D366]/10 flex items-center justify-center hover:bg-[#25D366] transition-all group border border-[#25D366]/10" title="Invitación a Plan Anual">
                                    ...
                                    <button onClick={() => sendWhatsAppOfferReiteration(u, isUSD)} className="w-7 h-7 rounded-lg bg-orange-500/10 flex items-center justify-center hover:bg-orange-500 transition-all group border border-orange-500/10" title="Reiteración de Oferta">
                                    ...
                                    <button onClick={() => sendWhatsAppLastOffer(u, isUSD)} className="w-7 h-7 rounded-lg bg-purple-500/10 flex items-center justify-center hover:bg-purple-500 transition-all group border border-purple-500/10" title="Última Oferta (Cierre)">
                ...
                .map((u, idx) => {
                  ...
                  return (
                    ...
                    >
                       ...
                       <div>
                          <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${u.tier?.includes('plan_max') || u.tier?.includes('duo') ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' : 'bg-blue-500/20 text-blue-400 border border-blue-500/30'}`}>
                             {u.tier?.replace('duo', 'PLAN MAX').replace('tareas', 'ENFOQUE').toUpperCase()} ({u.tier?.includes('habitos') || u.tier?.includes('enfoque') || u.tier?.includes('tareas') ? (u.is_usd ? '$ 7.90' : 'S/ 9.90') : (u.is_usd ? '$ 13.90' : 'S/ 12.90')})
                          </span>
                       </div>
