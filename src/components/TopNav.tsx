...
interface Props {
  page: 'tracker' | 'dashboard' | 'planner' | 'finances';
  setPage: (p: 'tracker' | 'dashboard' | 'planner' | 'finances') => void;
  userEmail: string;
  userTier?: string;
  isPaid?: boolean;
}
...
export default function TopNav({ page, setPage, userEmail, userTier, isPaid }: Props) {
  ...
  const tabs = [
    { id: 'tracker' as const, label: t('tracker_tab') },
    { id: 'planner' as const, label: t('planner_tab') },
    ...
    <nav className="bg-app-surface border-b border-app-border sticky top-0 z-[999] pt-10 sm:pt-0 flex flex-col">
      <div className="px-3 sm:px-6 flex items-center w-full">
        ...
        <button 
          ...
          className="sm:hidden p-2 -ml-2 mr-2 text-app-text hover:bg-app-surface2 rounded-lg transition-colors flex-shrink-0 -translate-y-3 sm:translate-y-0"
        >
        ...
        {/* Desktop Tabs */}
        <div className="hidden sm:flex overflow-x-auto no-scrollbar scroll-smooth flex-1">
          {tabs.map((tab) => {
            ...
            return (
              <button
                key={id}
                onClick={() => setPage(id)}
                className={`px-3 sm:px-5 py-4 text-[10px] sm:text-[11px] font-black uppercase tracking-widest border-b-2 transition-all whitespace-nowrap flex items-center gap-2
                  ${page === id
                    ? 'text-brand-green border-brand-green'
                    : 'text-app-text3 border-transparent hover:text-app-text hover:border-app-text3/30'}`}
              >
                {icon && icon}
                {label}
      ...
      {isMenuOpen && (
        <div className="sm:hidden w-full bg-app-surface border-t border-app-border shadow-lg absolute top-full left-0 animate-in slide-in-from-top-2 duration-200">
          {tabs.map((tab) => {
            const { id, label, icon } = tab;
            const isLocked = isPaid && (
              (id === 'planner' && userTier === 'habitos') ||
              ...
              <button
                ...
                className={`w-full text-left px-6 py-4 text-[11px] font-black uppercase tracking-widest transition-all border-b border-app-border last:border-0 flex items-center justify-between
                  ${page === id
                    ...
                    : 'text-app-text3 border-l-4 border-l-transparent'}`}
              >
