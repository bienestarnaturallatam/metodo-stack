...
export default function PlannerPanel({ userId }: { userId: string }) {
  ...
  const shareToWhatsApp = () => {
    ...
    const message = `Hola, esta es mi lista de TAREAS CRÍTICAS del día del Método STACK:\n\n${criticalTasks || 'Sin tareas críticas pendientes.'}\n\n¡Formato táctil y claro activado!\n¡Fricción cero activada!`;
        ...
        <header className="mb-12 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          <div>
            ...
            <div className="flex items-center gap-4">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-red-500">Pilar 2: Dinámicas</p>
        ...
        <div className="mb-12 relative group">
          ...
          <div className="relative flex flex-col sm:flex-row gap-4 p-6 bg-gray-900/90 rounded-[24px] border border-gray-800 backdrop-blur-xl">
            <div className="flex-1 flex items-center gap-4">
              ...
              <input 
                type="text"
                placeholder="Añadir Tarea Crítica para hoy..."
        ...
        <div className="space-y-6">
          {tasks.length === 0 ? (
            <div className="py-24 text-center rounded-[40px] border-2 border-dashed border-gray-900 bg-gray-950/50">
              ...
              <h2 className="text-3xl font-black text-gray-300 mb-2">Inicia tu OS Personal:</h2>
              <p className="text-red-500 font-bold uppercase tracking-widest animate-pulse">Añade tu primera Tarea Crítica 🔴</p>
            </div>
          ) : (
            tasks.map(task => (
              <div 
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    <span className="text-[9px] font-black uppercase tracking-widest text-red-500 px-2 py-0.5 rounded bg-red-500/10">CRÍTICO</span>
        ...
        {tasks.length > 0 && (
          <div className="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="p-6 bg-gray-900/50 rounded-2xl border border-gray-800 text-center">
              <p className="text-[9px] font-black text-gray-500 uppercase mb-1">Impacto Diario</p>
              <p className="text-2xl font-black text-red-500">{Math.round((tasks.filter(t => t.done).length / tasks.length) * 100)}%</p>
            </div>
            <div className="p-6 bg-gray-900/50 rounded-2xl border border-gray-800 text-center">
              <p className="text-[9px] font-black text-gray-500 uppercase mb-1">Completadas</p>
              <p className="text-2xl font-black text-emerald-500">{tasks.filter(t => t.done).length}</p>
            </div>
            <div className="p-6 bg-gray-900/50 rounded-2xl border border-gray-800 text-center">
              <p className="text-[9px] font-black text-gray-500 uppercase mb-1">Pendientes</p>
